...
NVIDIA's NR runtime checks which module NGX calls RETURN into (the return
address's image on disk) and rejects callers that are not literally named
``nvngx.dll``. This module emits a small position-independent DLL of that
name whose four exports are thunks: ``fwd_set_slots`` parks the real NGX
function addresses in .data slots, and ``fwd_create`` / ``fwd_evaluate`` /
``fwd_release`` forward to the parked target with a real ``call`` (never a
tail ``jmp``), so the return address on the stack points back into this
image and the caller check passes.
...
concept is credited there). The bytes below are written from the public x64
instruction encodings and the documented PE/COFF layout.
"""
...
FORWARDER_EXPORTS = ("fwd_create", "fwd_evaluate", "fwd_release", "fwd_set_slots")
...
def build_shim_dll(image_base=DEFAULT_IMAGE_BASE, dll_name="nvngx.dll"):
    ...
    create_rva = text.emit_call_thunk(0, 3)
    evaluate_rva = text.emit_call_thunk(1, 3)
    release_rva = text.emit_call_thunk(2, 3)
    set_slots_rva = text.emit_set_slots()
    text.pad_to(16)

    eat_rva = {"fwd_create": create_rva, "fwd_evaluate": evaluate_rva,
               "fwd_release": release_rva, "fwd_set_slots": set_slots_rva}
...
def write_shim(directory, dll_name="nvngx.dll"):
