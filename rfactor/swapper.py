...
from .engine.analyzer import ReActorFaceAnalysis
from .engine.face_objects import Face
from .engine.inswap import INSwapper
...
def _getAnalysisModel_locked(det_size=(640, 640)):
    ANALYSIS_MODEL = ANALYSIS_MODELS[str(det_size[0])]
    if ANALYSIS_MODEL is None:
        ANALYSIS_MODEL = ReActorFaceAnalysis(
            name="buffalo_l", providers=resolve_providers(), root=insightface_path
        )
